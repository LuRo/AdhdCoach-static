...
export const ComplexitySummaryCard = ({ gaugeAngle, gaugeTransition, onRecalculate, snapshot }: Props) => {
  ...
  const statusLabel =
    snapshot.status === "Overloaded"
      ? copy.ui.complexitySummaryCard.levels.high
      : snapshot.status === "Moderate"
        ? copy.ui.complexitySummaryCard.levels.moderate
        : copy.ui.complexitySummaryCard.levels.light;
    ...
    <SectionCard className="px-3 py-3 px-md-4 d-flex align-items-center gap-3">
      <div className="flex-grow-1 complexity-card">
        <div className="d-flex justify-content-between align-items-center gap-2 mb-2">
          <span className="fw-semibold">{copy.ui.complexitySummaryCard.title}</span>
        ...
        <div
          ...
          role="button"
          tabIndex={0}
          aria-label={copy.ui.complexitySummaryCard.recalculateAria}
        ...
        >
          <svg viewBox="0 0 220 140" aria-hidden="true">
            <defs>
              <linearGradient
                id="gauge-mid-gradient"
                x1="75"
                y1="49.4"
                x2="145"
                y2="49.4"
                gradientUnits="userSpaceOnUse"
                gradientTransform="rotate(20 110 49.4)"
              >
            ...
            <g
              id="complexity-needle"
              className="gauge-needle"
              style={{ transform: `rotate(${gaugeAngle}deg)`, transition: gaugeTransition, WebkitTransform: `rotate(${gaugeAngle}deg)`, WebkitTransition: gaugeTransition, willChange: "transform" }}
            >
          ...
          <div className="gauge-scale" aria-hidden="true">
            <span>{copy.ui.complexitySummaryCard.levels.light}</span>
            <span>{copy.ui.complexitySummaryCard.levels.moderate}</span>
            <span>{copy.ui.complexitySummaryCard.levels.high}</span>
          </div>
        </div>

        <div className="d-flex align-items-center justify-content-between gap-2 mt-3">
          <span className="small text-secondary">{copy.ui.complexitySummaryCard.threshold}</span>
          <CoachBadge className="px-3 py-2">
            {snapshot.total} {copy.ui.complexitySummaryCard.pointsLabel}
          </CoachBadge>
        </div>
      </div>
    </SectionCard>
