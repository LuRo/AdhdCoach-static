...
export const PomodoroOverlay = ({ onClose, onStart, onStop, task }: Props) => {
  ...
  return createPortal(
    <>
      <div className="pomodoro-overlay" role="dialog" aria-modal="true" aria-label={t("Pomodoro timer overlay")} data-testid="today-pomodoro-overlay">
        <div className="pomodoro-overlay-card">
          <div className="d-flex justify-content-between align-items-start gap-3 mb-3">
            <div>
              <p className="text-uppercase small text-secondary mb-1">{t("Pomodoro")}</p>
              <h2 className="h4 mb-1">{task.title}</h2>
              <p className="text-secondary mb-0">{t("{{minutes}} minute cycle", { minutes: task.pomodoroMinutes })}</p>
            </div>
          ...
          <div className="d-flex justify-content-center gap-2 mb-3">
            {DURATION_OPTIONS.map((minutes) => (
              <CoachButton key={minutes} type="button" variant={selectedMinutes === minutes ? "primary" : "outline"} className="rounded-pill px-2 py-1" testId={`today-pomodoro-duration-${minutes}-button`} onClick={() => setSelectedMinutes(minutes)}>
                {minutes}m
              </CoachButton>
            ))}

            <CoachButton type="button" variant="primary" className="rounded-pill px-3 py-1" testId="today-pomodoro-start-button" onClick={() => onStart(task.id, selectedMinutes)}>
              <i className="bi bi-play-fill" /> {t("Start")}
            </CoachButton>
          </div>
          ...
          <div className="pomodoro-overlay-timer">
            ...
            <div className="pomodoro-overlay-time-row">
              ...
              <CoachButton type="button" variant="outline" className="rounded-pill px-3 py-1" disabled={!task.timerRunning} testId="today-pomodoro-stop-button" onClick={() => onStop(task.id)}>
